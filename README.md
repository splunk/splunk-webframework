...
The Splunk Application Framework uses the [Django web framework](https://www.djangoproject.com/), the [Splunk SDK for Python](https://github.com/splunk/splunk-sdk-python), and the [Splunk SDK for JavaScript](https://github.com/splunk/splunk-sdk-javascript). The framework also depends on a few JavaScript libraries for the client-side of code, such as Backbone.js for eventing, and jQuery for working with the document object model (DOM).

**Note** Significant changes have been made since the Preview release, such as names of components and APIs and the way components are instantiated in JavaScript. Any code you created for the Preview of the Splunk Web Framework will not work with the GA release. Overall concepts of the framework remain the same, so see the code examples on the [Splunk Developer Portal](http://dev.splunk.com/view/app-framework/SP-CAAAEMA) and in the **/$WEBFRAMEWORK_HOME/server/apps/** directory.
...
* For information about the framework components, see the [Splunk Web Framework Reference](http://docs.splunk.com/Documentation/WebFramework).
...
* **The Splunk Web Framework**: The framework is available as a ZIP file from GitHub and as a Git repository.
...
2. Open a command prompt, navigate to the directory where you unzipped the framework (the **$WEBFRAMEWORK_HOME**).
...
The first time you run the Splunk Web Framework, you'll see the option to run the Quick Start, or log in using your Splunk credentials. The Quick Start is a great way to get a feel for what you can do with the Framework. Once you complete the Quick Start or log in, you'll see the framework home page, which lists all of the framework apps on your system.
...
The sample data is in a CSV file at **$WEBFRAMEWORK_HOME/server/apps/quickstartfx/splunkd/lookups/splunkdj.demo.dataset.csv**. You can include this data file in any framework app by creating a /lookups directory under **$WEBFRAMEWORK_HOME/server/apps/<*your_app_name*>/splunkd/** and copying the CSV file there.
...
3. Start the framework. At the command prompt, navigate to **$WEBFRAMEWORK_HOME**.
